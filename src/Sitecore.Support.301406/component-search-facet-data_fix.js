XA.component.search.facet.data = (function ($, document) {
    ...
    var FacetDataModel = Backbone.Model.extend({
        ...
        filterFacetData: function (hashObj) {
            ...
            for (var signature in data) {
                if (data.hasOwnProperty(signature)) {
					...
					//get itemID parameter form Search Results rendering //fix 301406
                    itemid = this.getSearchResultsItemId(signature);
		...
		//fix 301406
		getSearchResultsItemId: function(signature) {
            var searchResultsModel = this.getSearchResultsModelBySignature(signature);
            if (typeof searchResultsModel !== "undefined") {
				if (searchResultsModel.attributes.dataProperties.itemid!=="undefined")
				{
					return searchResultsModel.attributes.dataProperties.itemid;
				}
            }
            return "";
        },
